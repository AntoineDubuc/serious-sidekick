...
| `structural` | research → plan, research → mock-ups, mock-ups → plan | Match by explicit section reference or named task. An upstream finding "Rate limiting required at API gateway" must appear as a dedicated task, section, or table row in the downstream artifact — not buried in prose or parenthetical mentions. |
