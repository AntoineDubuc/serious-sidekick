...
> **Last updated:** 2026-05-18 · **Current version:** v1.8.0 (+ PR #2 voice retrofit Phases 2-6 pending merge) · **Claude Code:** v2.1.143 (latest — confirmed against the official Anthropic changelog on 2026-05-18; no new releases since the previous tracking update on 2026-05-16). Tracked feature highlights: v2.1.111 (`/ultrareview`, `/less-permission-prompts`, xhigh effort), v2.1.117 (fork-subagent externally enabled, 1M-context Opus 4.7 fix, default effort raised), v2.1.118 (hooks invoke MCP tools, fork-subagent GA), v2.1.121 (PostToolUse `updatedToolOutput` for all tools — but broken at runtime, see Issue #54196), v2.1.126 (gateway model discovery, `claude project purge`), v2.1.139 (`claude agents` view, `/goal` command, hook `args:` exec form, hook `continueOnBlock`), v2.1.142 (plugins with root-level SKILL.md surfaced), v2.1.143 (plugin dependency enforcement, stop-hook 8-block cap). See §A of `CLAUDE_CODE_CHANGELOG.md` for the condensed digest.
