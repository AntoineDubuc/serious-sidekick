...
# Clean up any stale breadcrumbs
cd "$PROJECT_ROOT"
rm -f .active-code .active-review .active-plan .active-scope .active-research .active-conversation
...
echo "$REVIEW_DIR" > .active-review
bash .claude/skills/serious-review/hooks/check-verdict.sh 2>"/tmp/check-verdict-output.txt"
EXIT_CODE=$?
rm -f .active-review
...
bash .claude/skills/serious-plan/hooks/check-extraction.sh 2>"/tmp/check-extraction-output.txt"
EXIT_CODE=$?
rm -f .active-plan
...
echo "$SCOPE_DIR" > .active-scope
bash .claude/skills/serious-scope/hooks/check-manifest.sh 2>"/tmp/check-manifest-output.txt"
EXIT_CODE=$?
rm -f .active-scope
...
echo "$RESEARCH_DIR" > .active-research
bash .claude/skills/serious-research/hooks/capture-research.sh 2>"/tmp/capture-research-output.txt"
EXIT_CODE=$?
...
VERDICT_FILE="Research/features/mechanical-enforcement-gap/plans/harden-existing-hooks/review_verdict.md"
...
SCOPE_EXTRACT="Research/features/mechanical-enforcement-gap/_extracted_items.md"
PLAN_EXTRACT="Research/features/mechanical-enforcement-gap/plans/harden-existing-hooks/_extracted_items.md"
...
MANIFEST_VERIFIED=$(head -20 "Research/features/mechanical-enforcement-gap/manifest.md" | grep "^verified:" | head -1)
PLAN_VERIFIED=$(head -20 "Research/features/mechanical-enforcement-gap/plans/harden-existing-hooks/implementation_plan.md" | grep "^verified:" | head -1)
...
PLAN_FILE="Research/features/mechanical-enforcement-gap/plans/harden-existing-hooks/implementation_plan.md"
CITATION_COUNT=$(grep -c '\[Source:' "$PLAN_FILE" || true)
ITEM_COUNT=$(grep -cE '^\s*[0-9]+\.' "Research/features/mechanical-enforcement-gap/plans/harden-existing-hooks/_extracted_items.md" || true)
...
for agent in implementer reviewer test-runner runtime-checker qa; do
  AGENT_FILE=".claude/agents/serious-code-${agent}.md"
...
CODE_SKILL=".claude/skills/serious-code/SKILL.md"
PLAN_SKILL=".claude/skills/serious-plan/SKILL.md"
...
TIMEOUT_30=$(grep -c '"timeout": 30' ".claude/settings.json" || true)
TIMEOUT_10=$(grep -c '"timeout": 10' ".claude/settings.json" || true)
