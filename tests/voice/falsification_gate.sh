...
# Initialize the final report
{
  ...
  echo ""
  echo "## Honest framing — read this before the per-example verdicts"
  echo ""
  echo "This report measures ONLY whether the safety-net validator hook (\`voice-gate.sh\`)"
  echo "fires on the 5 historical slop strings when they are fed verbatim into the hook."
  echo "It is a test of the safety net — NOT a test of whether the retrofitted skills"
  echo "produce clean output now. The hook regex was authored knowing these patterns, so"
  echo "this measurement is partially circular by design (a self-test of the safety net)."
  echo ""
  echo "The live-touchpoint replay that plan AC1 actually asked for (does the retrofitted"
  echo "skill produce clean output today?) is captured separately in"
  echo "\`evidence/falsification-live-replay-evidence.md\` — that artifact was authored"
  echo "by a runtime-verification sub-agent on 2026-05-17 after the retro-four-worker pass"
  echo "and is the substantive 'retrofit moved the needle' evidence."
  echo ""
  echo "Both measures pass the ≥4 of 5 threshold. Read the live-replay file for the"
  echo "claim about retrofit behavior; this file just confirms the safety net is wired."
  echo ""
  echo "## Per-example verdicts (Part A — safety-net regex coverage)"
  echo ""
} > "$FINAL"
...

{
  ...
  if [ "$ACCEPTABLE_TOTAL" -ge 4 ]; then
    echo "## Gate verdict (Part A — safety-net): **PASS**"
    echo ""
    echo "Safety net catches the 5 historical slop strings. This proves the hook regex is"
    echo "wired correctly against the patterns it was built for; it does NOT prove the"
    echo "retrofitted skills produce clean output. For that claim, see"
    echo "\`evidence/falsification-live-replay-evidence.md\` (Part B)."
  else
    echo "## Gate verdict (Part A — safety-net): **FAIL (BLOCKED)**"
    echo ""
    echo "Safety net failed to catch ≥4 of 5 historical strings. Cannot proceed."
  fi
} >> "$FINAL"
